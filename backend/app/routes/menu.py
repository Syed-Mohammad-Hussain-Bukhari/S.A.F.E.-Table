...
async def delete_menu_item(
    ...
    A hard delete would orphan historical order line items that reference
    this id, breaking sales reports."""
    db = get_database()
    if not ObjectId.is_valid(item_id):
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "Invalid item ID")
    ...
    if result.matched_count == 0:
        ...
        if not existing:
            raise HTTPException(status.HTTP_404_NOT_FOUND, "Menu item not found")
        raise HTTPException(status.HTTP_409_CONFLICT, "Menu item already deleted")
