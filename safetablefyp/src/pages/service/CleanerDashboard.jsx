...
>>>>>>> 3cb3c76 (Update backend changes by Hashaam via Claude Code)
  ...
  // Sound Alert Logic
  useEffect(() => {
    // Check for new pending clean requests
    const currentPending = requests.filter((r) => r.type === 'clean' && r.status === 'pending');

    if (currentPending.length > prevRequestsLength.current) {
      // Find the newest request to announce
      const newRequest = currentPending.sort((a, b) => new Date(b.requestedAt).getTime() - new Date(a.requestedAt).getTime())[0];

      // Only announce if it's very recent (within last 10 seconds) to avoid blast on load
      ...
      <div className="px-6 py-4 border-b border-border bg-card/30">
            ...
            <div className="bg-card/50 border-b border-border px-6 py-2 flex items-center justify-between text-xs font-mono text-muted-foreground">
                <div className="flex gap-6">
                    <span className="flex items-center gap-2"><Activity className="w-3 h-3 text-primary" /> Active: <span className="text-foreground font-bold">{stats.pending}</span></span>
                </div>
                <div className="flex items-center gap-2">
                    <span className="flex items-center gap-1">Status: <span className={`font-bold ${stats.pending === 0 ? 'text-green-500' : 'text-purple-500'}`}>{stats.pending === 0 ? 'Clear' : 'Active'}</span></span>
                </div>
            </div>
                ...
                return (
                  <motion.div
                    ...
                    layout>
                                            ...
                                            <Card className="p-6 bg-card border-purple-500/30 shadow-lg shadow-purple-500/5 relative overflow-hidden h-full flex flex-col justify-between hover-glow-purple">
                                                ...
                                                <Button
                        className="w-full bg-purple-600 hover:bg-purple-500 text-white font-medium py-6 text-lg shadow-lg shadow-purple-900/20 transition-all active:scale-[0.98]"
