...
=======
  // Initial load + 1Hz tick for elapsed time.
  ...
  // Live updates from WebSocket: re-pull on every event so the store and the
  // server stay tightly synced. (We could patch the array in place too but
  // a refresh is fine at restaurant scale.)
...
=======
      ...
      <div className="bg-card/50 border-b border-border px-6 py-2 flex items-center justify-between text-xs font-mono text-muted-foreground">
        <div className="flex gap-6">
          <span className="flex items-center gap-2"><Activity className="w-3 h-3 text-primary" /> Active: <span className="text-foreground font-bold">{stats.activeCount}</span></span>
          {loading && <span className="flex items-center gap-2"><Loader2 className="w-3 h-3 animate-spin" /> Syncing</span>}
        </div>
        <div className="flex items-center gap-2">
          <span className="flex items-center gap-1">Status: <span className={`font-bold ${stats.activeCount === 0 ? "text-green-500" : "text-orange-500"}`}>{stats.activeCount === 0 ? "Clear" : "Busy"}</span></span>
        </div>
...
=======
                className={`px-4 py-1.5 rounded-md text-sm font-bold transition-all whitespace-nowrap ${
                  isActive ? "bg-primary/20 text-primary border border-primary/50 shadow-sm" :
        ...
        <div className="flex gap-4 items-center w-full md:w-auto shrink-0">
          <div className="relative flex-1 md:w-64">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <Input placeholder="Search orders..." className="pl-9 bg-input border-input focus:border-primary w-full" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          </div>

          <Select value={sortBy} onValueChange={setSortBy}>
            <SelectTrigger className="w-[140px] bg-input border-input">
              <div className="flex items-center gap-2"><ArrowUpDown className="w-4 h-4 text-muted-foreground" /><SelectValue placeholder="Sort" /></div>
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="newest">Newest First</SelectItem>
              <SelectItem value="oldest">Oldest First</SelectItem>
      ...
      <ScrollArea className="flex-1 p-6 pt-2">
        {paginatedOrders.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
            <AnimatePresence mode="popLayout">
              {paginatedOrders.map((order) => {
                ...
                return (
                  <motion.div key={order.orderId} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative group" onClick={() => setSelectedOrder(order)}>
                    <Card className={`h-full border bg-card/60 backdrop-blur-sm overflow-hidden cursor-pointer hover-glow ${cfg.border} ${isLate ? "ring-2 ring-destructive/50" : ""}`}>
                      <div className={`p-3 border-b ${cfg.border} ${cfg.bg} flex justify-between items-center`}>
                        <div className="flex items-center gap-2"><StatusIcon className={`w-4 h-4 ${cfg.color}`} /><span className={`text-sm font-bold ${cfg.color}`}>{cfg.label}</span></div>
                        <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded ${isLate ? "bg-destructive/20 text-destructive-foreground" : "bg-muted text-muted-foreground"}`}>
                          <Clock className="w-3 h-3" /><span className="text-xs font-mono font-bold">{elapsed}m</span>
                        </div>
                      </div>
                      {order.status !== "ready" && (
                        <div className="h-1 w-full bg-slate-800"><div className={`h-full ${progressColor} transition-all duration-1000`} style={{ width: `${progress}%` }} /></div>
                      )}
                      <div className="p-5">
                        <div className="flex justify-between items-start mb-4">
                          <h3 className="text-2xl font-bold text-white">Table {order.tableNumber}</h3>
                          <Badge variant="outline" className="text-slate-500 border-slate-700 font-mono text-[10px]">#{order.orderId.split("-")[1]}</Badge>
                        </div>
                        <div className="space-y-3">
                          {order.items.slice(0, 4).map((item, idx) => (
                            <div key={idx} className="flex justify-between items-start text-sm">
                          ...
                          {order.items.length > 4 && (<p className="text-xs text-slate-500 italic pt-1">+ {order.items.length - 4} more items...</p>)}
                        </div>
                        {isLate && (<div className="absolute top-2 right-2 animate-pulse"><AlertCircle className="w-5 h-5 text-red-500" /></div>)}
                      </div>
                    </Card>
                  </motion.div>
                );
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="h-full flex flex-col items-center justify-center text-muted-foreground py-20">
            <motion.div animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} className="p-8 rounded-full bg-gradient-to-br from-card via-muted/50 to-card mb-6 border-2 border-border">
              <ChefHat className="w-16 h-16 text-muted-foreground/50" />
            </motion.div>
            <h3 className="text-2xl font-bold text-muted-foreground mb-2">All Clear in the Kitchen</h3>
            <p className="text-muted-foreground/80 text-center max-w-md">{statusFilter === "all" || statusFilter === "pending" ? "Waiting for orders... The kitchen is ready!" : `No ${statusFilter} orders at the moment.`}</p>
          </motion.div>
        )}
      </ScrollArea>
      ...
      <OrderDetailsDialog order={selectedOrder} open={!!selectedOrder} onOpenChange={(open) => !open && setSelectedOrder(null)} onUpdateStatus={updateOrderStatus} />
    </div>
  );
};
...
const OrderDetailsDialog = ({ order, open, onOpenChange, onUpdateStatus }) => {
  ...
  else if (order.status === "preparing") { nextStatus = "ready"; buttonLabel = allChecked ? "Mark as Ready" : "Verify Items Before Completion"; isButtonDisabled = !allChecked; }
  else if (order.status === "ready") { nextStatus = "completed"; buttonLabel = "Mark Completed"; }

  const handleSubmit = async () => {
    if (!nextStatus) return;
    try { await onUpdateStatus(order.orderId, nextStatus); onOpenChange(false); }
      ...
      <DialogContent className="bg-background border-border text-foreground max-w-2xl max-h-[90vh] flex flex-col p-0 overflow-hidden sm:max-w-2xl">
        <DialogHeader className="p-6 pb-2 bg-muted/30 border-b border-border">
          <div className="flex justify-between items-center pr-8">
            <div>
              <DialogTitle className="text-2xl font-bold flex items-center gap-3">
                Table #{order.tableNumber}
                <Badge variant="secondary" className="bg-muted text-muted-foreground border border-border">{order.status.toUpperCase()}</Badge>
              </DialogTitle>
              <p className="text-muted-foreground font-mono text-sm mt-1">{order.orderId}</p>
            </div>
          </div>
        ...
        <ScrollArea className="flex-1 p-6 pt-2">
          <div className="space-y-6">
            {order.status === "preparing" && (
              <div className="bg-blue-950/20 border border-blue-900/30 rounded-lg p-4 mb-6">
                <h4 className="text-blue-200 font-bold mb-2 flex items-center gap-2"><AlertCircle className="w-4 h-4" /> Verify each item</h4>
            ...
            <div className="space-y-4">
              {order.items.map((item, idx) => {
                ...
                return (
                  <div key={uid} className={`flex items-start gap-4 p-4 rounded-xl border transition-all ${isPreparing ? "cursor-pointer hover:border-primary/50" : ""} ${isChecked ? "bg-green-500/10 border-green-500/30 opacity-60" : "bg-card border-border hover:bg-muted/50"}`} onClick={() => toggleItem(uid)}>
                    {isPreparing && <Checkbox checked={isChecked || false} className="mt-1 data-[state=checked]:bg-green-500 data-[state=checked]:border-green-500" />}
                    <div className="flex-1">
                      <div className="flex justify-between">
                        <h3 className={`font-bold text-lg ${isChecked ? "text-muted-foreground line-through" : "text-foreground"}`}>{item.quantity}x {item.name}</h3>
                        <span className="text-muted-foreground font-mono">${item.price}</span>
                      </div>
                      {item.notes && (<div className="mt-2 bg-orange-500/10 text-orange-400 px-3 py-1.5 rounded text-sm font-medium inline-block border border-orange-500/20">Note: {item.notes}</div>)}
                    </div>
                  </div>
                );
              })}
        ...
        <DialogFooter className="p-6 bg-muted/30 border-t border-border">
          <div className="flex w-full gap-4">
            ...
            {order.status !== "completed" && (
              <Button disabled={isButtonDisabled} className="flex-[2] text-lg font-bold" onClick={handleSubmit}>{buttonLabel}</Button>
            )}
          </div>
        </DialogFooter>
      </DialogContent>
